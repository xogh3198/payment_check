...
object NhDepositParser {
    private val nhPackageNames = setOf(
        "nh.smart.banking",
        "com.nonghyup.nhallonebank",
        "nh.smart.nhcok",
    )
    ...
    private val explicitDepositorPattern = Regex(
        "(?:입금자|보낸\\s*분|보낸\\s*사람|의뢰인|적요|거래내용)\\s*[:：]?\\s*([^\\n|,/]+)",
    )
    ...
    fun parse(
        ...
        if (!looksLikeNh(packageName, rawText) || !rawText.contains("입금")) return null
    ...
    private fun looksLikeNh(packageName: String, rawText: String): Boolean {
        return packageName in nhPackageNames || containsNhMarker(rawText)
    }

    private fun containsNhMarker(rawText: String): Boolean {
        return rawText.contains("[NH]", ignoreCase = true) ||
            rawText.contains("NH농협", ignoreCase = true) ||
            rawText.contains("농협은행", ignoreCase = true) ||
            rawText.contains("농축협", ignoreCase = true) ||
            rawText.contains("농협", ignoreCase = true) ||
            rawText.contains("NH스마트뱅킹", ignoreCase = true) ||
            rawText.contains("NH올원뱅크", ignoreCase = true) ||
            rawText.contains("올원뱅크", ignoreCase = true) ||
            rawText.contains("NH콕뱅크", ignoreCase = true) ||
            rawText.contains("콕뱅크", ignoreCase = true)
    }
